        ...
        if ($sending_server->custom && $sending_server->type == 'http') {
            ...
            switch ($gateway_name) {
                ...
                case SendingServer::TYPE_TWILIO:
                    ...
                    try {
                        ...
                        if ($get_response->status == 'queued' || $get_response->status == 'accepted') {
                            ...
                            Log::warning('Twilio SMS status issue', [
                                'sender_id' => $sender_id,
                                'phone' => $phone,
                                'message_sid' => $get_response->sid,
                                'status' => $get_response->status
                ...
                case SendingServer::TYPE_SIGNALWIRE:
                    Log::info("Reached signalwire sending");
                    $parameters = [
                        'From' => '+' . $data['sender_id'],
                        'Body' => $message,
                        'To'   => '+' . $phone,
                ...
                case SendingServer::TYPE_SMSALA:
                    ...
                    if (isset($sending_server->c1)) {
                        $parameters['pe_id'] = $sending_server->c1;
                    }
                ...
                case SendingServer::TYPE_GOIP:
                    ...
                    try {
                        $get_sms_status = file_get_contents(
                            $gateway_url . "?u=$sending_server->username&p=$sending_server->password&l=1&n=$phone&m=$message",
                            false,
                            $context
                        );
