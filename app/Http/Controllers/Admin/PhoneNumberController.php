...
class PhoneNumberController extends AdminBaseController
{
    ...
    public function create(): Factory|View|Application
    {
        ...
        $assistants=$this->fetchAssistants();


        return view('admin.PhoneNumbers.create', compact('breadcrumbs', 'currencies', 'customers', 'sending_servers','assistants'));
    }
    ...
    public function show(PhoneNumbers $number): \Illuminate\Contracts\View\View|Factory|RedirectResponse|Application
    {
        ...
        if (isset($capabilities) && is_array($capabilities) && count($capabilities) > 0) {
            return view('admin.PhoneNumbers.show', compact('breadcrumbs', 'number', 'customers', 'currencies', 'capabilities', 'sending_servers','assistants'));
        }
    ...
    public function store(StoreNumber $request, PhoneNumbers $numbers): RedirectResponse
    {
        ...
        $t = $this->numbers->store($request->input(), $numbers::billingCycleValues());
    ...
    private function fetchAssistants()
{
    // Define the API endpoint
    $url = 'https://internaltools.godspeedoffers.com/api/all-assistants';
    ...
    if ($response->successful()) {
        ...
        $assistants = $response->json('data'); // Extract the 'data' key from the response

        // Map the result to an array of name and openAI_id
        return collect($assistants)->map(function ($assistant) {
            return [
                'name' => $assistant['name'],
    ...
    throw new \Exception('Failed to fetch assistants: ' . $response->body());
}
}
