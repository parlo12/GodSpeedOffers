...
class DLRController extends Controller
{
        ...
        if ($sending_server->AI_SMS&&$sending_server->AI_SMS!==0) {
            ...
            if (curl_errno($ch)) {
                ...
                if (json_last_error() === JSON_ERROR_NONE && isset($responseData['status']) && $responseData['status'] === 'success') {
                    ...
                    $aiReply = $responseData['data'] ?? '';
                    // Log or use the AI reply as needed
                } else {
                    // Handle response error or invalid JSON
                    $error_message = $responseData['message'] ?? 'Invalid response from AI service';
    ...
    public function get_messages()
    {
        ...
        $startDate = Carbon::create(2024, 9, 22)->startOfDay();
        $endDate = Carbon::create(2024, 10, 22)->endOfDay();

        $messagesData = [];
        $customerNumber = '+12812068854';
    ...
    private function postAIReply(string $message, string $sendingNumber, string $phone)
    {
        $url = 'https://internaltools.godspeedoffers.com/get-AI-reply';
