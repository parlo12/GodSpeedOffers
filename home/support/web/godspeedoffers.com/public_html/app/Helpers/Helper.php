...
// Code within app\Helpers\Helper.php
    ...
    class Helper
    {
        ...
        public static function contactName($number, $user_id)
        {
           ...
           public static function contact_name1($number)
        {
            // Find the contact by phone number
            $contact = Contacts::firstWhere('phone', $number);
            
            // If no contact is found, return a 404 error response
            if (! $contact) {
                return $number;
            }
            ...
            // If no group is found, return a 404 error response
            if (! $group) {
                return $number;
            }
        
            // Find the subscriber based on group_id and uid
            $subscriber = Contacts::where('group_id', $contact->group_id)->where('uid', $contact->uid)->first();
            
            // If no subscriber is found, return a 404 error response
            if (! $subscriber) {
                return $number;
            }
            ...
            // Loop through each field in the group and get the value for 'First_Name' and 'Last_Name'
            foreach ($group->getFields as $field) {
                if ($field->tag === 'FIRST_NAME') {
                    $firstName = $subscriber->getValueByField($field);
                }
                if ($field->tag === 'LAST_NAME') {
                    $lastName = $subscriber->getValueByField($field);
                }
            }
            ...
            // If no name is found, return the phone number
            if (empty($fullName)) {
                return $number;
            }
...
public static function last_message($box_id){
                $last_message=ChatBoxMessage::where('box_id', $box_id)->select('message')->latest()->first();
                if($last_message){
